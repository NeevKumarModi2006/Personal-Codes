...
bool checkbst(mnode* M) {
	...
	if (M->mptr.size() == 1)  return ( M->mptr[0]->data < M->data && checkbst(M->mptr[0]) );
	else return ( M->mptr[0]->data < M->data && M->mptr[1]->data > M->data && checkbst(M->mptr[0]) && checkbst(M->mptr[1]) );
}

bool checkheap(mnode* M) {
	if (M->mptr.size() > 2) return false;
	if (M->mptr.size() == 0) return true;
	if (M->mptr.size() == 1) return ( M->mptr[0]->data > M->data && checkheap(M->mptr[0]) );
	if (!(M->mptr[0]->data > M->data && M->mptr[1]->data > M->data)) return false;
	if (height(M->mptr[0]) != height(M->mptr[1])) return false;
	return checkheap(M->mptr[0]) && checkheap(M->mptr[1]);
}

vector<vector<pair<string, int>>> ans(25);
	...
	for (int i = 0; i < M->mptr.size(); i++) {
		if (M->mptr[i]) {
			if (!M->mptr[i]->mptr.empty() && checkbst(M->mptr[i])) {
				ans[level].push_back({"BST", M->mptr[i]->data});
				M->mptr[i] = NULL;
			} 
			else if (!M->mptr[i]->mptr.empty() && checkheap(M->mptr[i])) {
				ans[level].push_back({"Heap", M->mptr[i]->data});
				M->mptr[i] = NULL;
			} 
			else DFT(M->mptr[i], level + 1);
		}
	}
}
...
int main() {
	...
	for (int k = 0; k<ans.size(); k++) {
		if (!ans[k].empty()) {
			cout << k << " ";
			for (auto &i : ans[k])  cout << i.first << " " << i.second << " ";
					cout << endl;
		}
	}
